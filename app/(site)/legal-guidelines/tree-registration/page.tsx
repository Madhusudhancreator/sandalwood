import { getTreeRegistrationData } from "@/lib/sanity/queries/legalGuidelines";
...
export async function generateMetadata(): Promise<Metadata> {
  const data = await getTreeRegistrationData();
  return { title: data?.seo?.metaTitle ?? "Tree Registration", description: data?.seo?.metaDescription ?? "" };
}

export default async function TreeRegistrationPage() {
  const data = await getTreeRegistrationData();
  return (
    <>
      <PageHero title={data?.title ?? "Tree Registration"} breadcrumb={[{ label: "Home", href: "/" }, { label: "Legal Guidelines", href: "/legal-guidelines" }, { label: "Tree Registration" }]} />
