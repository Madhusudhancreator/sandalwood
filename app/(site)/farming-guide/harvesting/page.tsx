import { getHarvestingPageData } from "@/lib/sanity/queries/farmingGuide";
...
export async function generateMetadata(): Promise<Metadata> {
  const data = await getHarvestingPageData();
  return { title: data?.seo?.metaTitle ?? "Harvesting Guide", description: data?.seo?.metaDescription ?? "" };
}
...
export default async function HarvestingPage() {
  ...
  return (
    <>
      <PageHero title={data?.title ?? "Harvesting"} breadcrumb={[{ label: "Home", href: "/" }, { label: "Farming Guide", href: "/farming-guide" }, { label: "Harvesting" }]} />
