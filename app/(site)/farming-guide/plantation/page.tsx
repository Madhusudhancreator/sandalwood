import { getPlantationPageData } from "@/lib/sanity/queries/farmingGuide";
...
export async function generateMetadata(): Promise<Metadata> {
  const data = await getPlantationPageData();
  return { title: data?.seo?.metaTitle ?? "Plantation Guide", description: data?.seo?.metaDescription ?? "" };
}

export default async function PlantationPage() {
  const data = await getPlantationPageData();
  return (
    <>
      <PageHero title={data?.title ?? "Plantation"} breadcrumb={[{ label: "Home", href: "/" }, { label: "Farming Guide", href: "/farming-guide" }, { label: "Plantation" }]} />
